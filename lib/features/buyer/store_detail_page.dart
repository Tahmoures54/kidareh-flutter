...
class _StoreDetailPageState extends State<StoreDetailPage> {
  ...
  Widget _content(BuildContext context, Map<String,dynamic> data) {
    ...
    return ListView(padding:const EdgeInsets.fromLTRB(16,16,16,32),children:[
      ...
      if(products.isNotEmpty) ...[const SizedBox(height:24),Text('کالاهای فروشگاه',style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),const SizedBox(height:8),...products.map((p){ final id=int.tryParse(p['id']?.toString()??''); final price=p['price']; final priceText=price is num&&price>0?'${price.toStringAsFixed(0)} تومان':'قیمت توافقی'; return Card(child:ListTile(onTap:id==null?null:()=>context.push('/products/$id'),title:Text((p['name']??'کالا').toString(),maxLines:2,overflow:TextOverflow.ellipsis),subtitle:Text('$priceText • ${(p['status']??'ناموجود').toString()}'),trailing:id!=null?const Icon(Icons.chevron_left):null)); })],
    ]);
  }
}